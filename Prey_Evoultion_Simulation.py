    ...
    while running:
                ...
                if timer >= 3:
                    ...
                    if distance < PREDATOR_SIZE + PREY_SIZE:
                        if predator.can_reproduce(current_time):
                            ...
                            new_predator.time_without_food = current_time + random.uniform(0, 3)

                            new_predator_speed = PREDATOR_SPEED
        ...
        for prey in preys:
            ...
            if prey.can_reproduce(current_time):
                ...
                new_prey.x = prey.x
                new_prey.y = prey.y
                new_prey.last_reproduction_time = current_time + random.uniform(0, 5)
        ...
        if int(current_time) >= 30:
            ...
            x_values = np.arange(10, len(average_preys_speeds_every_second))
            sum_data = np.array(average_preys_speeds_every_second[10:])
            A_sum = np.vstack([x_values, np.ones(len(x_values))]).T
            m_sum, b_sum = np.linalg.lstsq(A_sum, sum_data, rcond=None)[0]

            print("Slope (Tilt) of the Graph for Prey's speed:", m_sum)

            plt.scatter(x_values, sum_data, label='Average Prey Speed')
            plt.plot(x_values, m_sum * x_values + b_sum, 'r', label='Optimized Line for Prey\'s speed')
            plt.title('Optimized Line for Prey\'s speed')
